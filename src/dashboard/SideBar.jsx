...
export default function SideBar() {
   ...
   let userName = user ? user.displayName ? user.displayName : user.email : "USER"; 
   if(user && user.photoURL) userImg = user.photoURL;
    return (
        <div>

            <button onClick={toggleMenu} data-drawer-target="separator-sidebar" data-drawer-toggle="separator-sidebar" aria-controls="separator-sidebar" type="button" className={`${isMenuOpen ? "hidden" : ""}inline-flex items-center p-2 mt-2 ms-3 text-sm text-gray-500 rounded-lg sm: hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600`}>
...
<aside id="separator-sidebar" className={`${isMenuOpen ? "" : "hidden"} fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0`} aria-label="Sidebar">
   ...
   <svg className="shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
               ...
               <span className="ms-3">Close drawer</span>
            </button>


   <a href="/" className="flex items-center ps-2.5 mb-5">
         <img src={userImg} className="rounded-full h-15 w-15 me-3" alt="Flowbite Logo" />
         <div className="flex flex-wrap">
         {user && <span>{String(userName).replace(/@\w[\w.-]*|\.com\b/g,'')}</span>}
         <span className="self-center text-xl font-semibold whitespace-nowrap dark:text-white">Home Page</span>
         </div>
      </a>
      ...
      <ul className="space-y-2 font-medium">
         ...
         <li>
            <a href="/admin/dashboard/upload" className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
             ...
             <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 15v2a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-2M12 4v12m0-12 4 4m-4-4L8 8"/>
               ...
               <span className="flex-1 ms-3 whitespace-nowrap">Upload Book</span>
               <span className="inline-flex items-center justify-center px-2 ms-3 text-sm font-medium text-gray-800 bg-gray-100 rounded-full dark:bg-gray-700 dark:text-gray-300">Pro</span>
            </a>
         </li>
         <li>
            <a href="/admin/dashboard/manage" className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
               <svg className="shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
